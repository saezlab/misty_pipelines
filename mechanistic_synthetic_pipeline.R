# This script produces the Misty results for the insilico mechanistic model appeared in the 
# GenomBiology paper.
...
# but we analyzed interactions from 1 up to 5 intermediate nodes
max_GT_depth = 1
...
# run the analysis independently for each cell type and without cell type information
for(i_analysis in 1:length(ct_results_folder)){
  ...
  joined_intra <- true.connections %>% filter(view == "intra") %>% 
    left_join(misty.results$importances %>% filter(view == "intra"), 
              by = c("node1" = "Predictor", "node2" = "Target")) %>% 
  ...
  joined_para <- true.connections %>% filter(view == "para") %>%
    left_join(misty.results$importances %>% filter(view == "para.10"), 
              by = c("node1" = "Predictor", "node2" = "Target")) %>%
    filter(!is.na(present), !is.na(Importance))
  
  # para with pre-filtering: only consider targets where the improvement of 
